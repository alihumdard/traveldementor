...
<style>
  .ds160-alert-row {
    ...
    animation: ds160Pulse 1.6s infinite;
  }
        ...
        <div class="row my-1 g-2 justify-content-start justify-content-md-end">
          ...
          <div class="col-lg-3 col-md-6 col-sm-12 d-flex justify-content-md-end justify-content-start ">
            <a href="{{ route('ds.add') }}">
              <button class="btn add-btn text-white"
                ...
                <span><i class="fa fa-plus me-2"></i>Add DS160</span>
              </button>
            </a>
          </div>
          <!-- Search Input -->
          <div class="col-lg-6 col-md-6 col-sm-12">
            <input type="text" id="search_input" class="form-control w-100"
              placeholder="Search Applicant Name or Category Name" style="height: 45px;" />
          </div>
        </div>
        <hr>
        <div class="px-2">
            ...
            <table id="qoute-table" class="display" style="width:100%">
              ...
              <tbody id="tableData">
                @foreach ($ds160 as $ds)
                  ...
                  <tr class="{{ $ds->ds160_alert_count > 0 ? 'ds160-alert-row' : '' }}" style="font-size: small;">

                    <td>{{ $loop->iteration ?? '' }}</td>

                    <td>
                      {{ ($ds->client->name ?? 'N/A') . (isset($ds->client->sur_name) ? ' ~ ' . $ds->client->sur_name : '') }}

                      @if($ds->ds160_alert_count > 0)
                        <i class="fas fa-file-invoice-dollar text-warning ms-1" title="USA Challan Expiry Alert"></i>
                      @endif
                    </td>
                    ...
                    <td>{{ $ds->ds160 ?? '' }}</td>
                    <td>
                      {{ $ds->challan_created == 'yes' ? 'Created' : 'Not Created' }}
                    </td>
