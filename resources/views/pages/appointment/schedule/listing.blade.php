              ...
              <tbody id="tableData">
                @foreach ($appointments ?? [] as $key => $appointment)
                  <tr class="{{ $appointment->scheduled_alert_count > 0 ? 'scheduled-alert-row' : '' }}"
                    ...
                    {{-- <td>{{\Carbon\Carbon::parse($appointment->bio_metric_appointment_date)->format('d-M-Y') ?? '' }}
                    </td> --}}
                    {{-- <td>{{\Carbon\Carbon::parse($appointment->transaction_date)->format('d-M-Y') ?? '' }}</td> --}}
                    <td>
                      {{ $appointment->transaction_date ? \Carbon\Carbon::parse($appointment->transaction_date)->format('d-M-Y') : ''  }}
                    </td>
                    <td class="">
...
@pushOnce('scripts')
  <script>
    ...
    var service_table = $('#qoute-table').DataTable({
      ...
      buttons: [
        ...
        },
        {
          ...
          exportOptions: {
            columns: [0, 1, 2, 3, 4] // Skips the 8th column (index 7)
          }
        },
      ],
      createdRow: function (row, data, dataIndex) {
        var appointmentDate = data[4]; // Column 4 = Appointment Date
        if (appointmentDate) {
          var today = new Date();
          var parts = appointmentDate.split('-'); // d-M-Y
          var formattedDate = new Date(parts[2], getMonthIndex(parts[1]), parts[0]);
    ...
    // Helper function to convert month short name to month index
    function getMonthIndex(monthShort) {
